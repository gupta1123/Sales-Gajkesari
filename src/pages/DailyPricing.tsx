...
const DailyPricingPage = () => {
    ...
    const fetchBrandData = async (start: string, end: string) => {
        try {
            const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/brand/getByDateRange?start=${start}&end=${end}`, {
                headers: {
            ...
            if (data.length === 0) {
                setNoDataMessage('No data available for the selected month and year. Please choose a different date.');
            } else {
                setNoDataMessage('');
            }
            setBrandData(data);
